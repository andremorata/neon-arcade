...
// A revisão Aurora nos assets evita reutilizar o tema antigo no cache de memória do WebKit.
...
const SHELL = [
  ...
  'assets/css/neon-theme.css?v=fullscreen-1',
  'assets/js/neon-core.js?v=fullscreen-1',
...
self.addEventListener('fetch', e => {
  ...
  // Revalida também o cache HTTP: fetch padrão pode devolver CSS antigo sem rede.
  const rede = fetch(req, ehCodigo(req) ? { cache: 'no-cache' } : {}).then(async res => {
